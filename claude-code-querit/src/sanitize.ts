...
/** Remove terminal escape sequences, unsafe controls, and bidi controls. */
...
/** Redact exact secret values before sanitizing text for an error surface. */
...
export function safeErrorMessage(error: unknown, secrets: readonly (string | undefined)[] = []): string {
  ...
  return redactSecrets(message, secrets) || "Unknown error";
}
